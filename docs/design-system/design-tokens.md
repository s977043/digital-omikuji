...
`design-system/tokens/*.json` を Digital Omikuji UI の唯一の設計 SSOT とし、React Native / NativeWind 実装はその投影先として扱います（アプリのバンドル対象 `design-system/index.ts` が直接 require するため、ソース配下に配置）。
