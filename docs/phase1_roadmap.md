...
## 子Issueリスト
- [x] #1 抽選中（DrawingOverlay）を追加して“儀式の間”を作る
- [x] #2 結果ページを巻物UIに刷新 + シェアで詳細解放（ゲート）
- [x] #3 Xシェア文言をテンプレ化（結果先頭/改行/ハッシュタグ/CTA）
- [x] #4 1日1回制限 + 履歴（昨日/カレンダー）で継続性を作る
- [x] #5 デザインサンプル（画面遷移/配色/タイポ）をdocsにまとめる
...
- [x] `DrawingOverlay` コンポーネントの作成
- [x] 状態遷移に `drawing` を追加 (idle -> shaking -> drawing -> revealed)
- [x] アニメの実装 (Moti/Reanimated)
- [x] Haptics/Sound の追加
- [x] 二重遷移防止の実装
- [x] アクセシビリティ対応
...
- [x] `utils/buildShareText.ts` の作成
- [x] 文字数制限を考慮したテキスト生成ロジックの実装
- [x] シェアボタンへの適用
- [x] URLへの計測パラメータ付与
...
- [x] 永続化の実装 (AsyncStorage/SQLite)
- [x] 日付判定ロジック（ローカル/タイムゾーン考慮）
- [x] 履歴画面の作成
- [x] 「本日の結果」再表示ロジック
- [x] デバッグ用日付操作機能
...
- [x] デザインサンプル画像の追加 (Placeholder added)
- [x] `design_guidelines.md` の作成
- [x] READMEからのリンク追加
